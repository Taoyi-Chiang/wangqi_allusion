...
# ----------------------------------------------------------------------------
# 1. 讀主 JSON，建立 main_lookup、sentence_order_map，並把每句的 tokens 轉成 token_to_terms
#    若該句已在句級比對中命中，則跳過不把它的 tokens 納入 token_to_terms
# ----------------------------------------------------------------------------
...
token_to_terms = {}      # token（字串） → list of term_info
all_tokens = set()
    ...
    for para in essay.get("段落", []):
        ...
        for grp in para.get("句組", []):
            ...
            for sentence in grp.get("句子", []):
                ...
                orig_sent = sentence.get("內容", "").strip()  # 原始句在 "內容" 欄位
                ...
                # 若此句已在句級 match，跳過不把它的 tokens 加入
                ...
                for token in token_list:
                    ...
                    token_to_terms.setdefault(tok, []).append(term_info)
                    all_tokens.add(tok)
                    term_order += 1
    ...
    for idx, seg in enumerate(segments):
        ...
        for n in NGRAM_RANGE:
            for gram in extract_char_ngrams(chars, n):
                found_ngrams.add(gram)

        # 逐一檢查哪些 n-gram 恰好與某個 token 完全相同
        for gram in found_ngrams:
            if gram not in all_tokens:
                continue

            for term_info in token_to_terms[gram]:
                key_sentence = (
                    term_info["賦家"],
                    term_info["賦篇"],
                    term_info["段落編號"],
